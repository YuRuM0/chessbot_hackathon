...
static size_t generate_simple_move(const struct position *pos, struct move *moves, int from_square, int file_offset, int rank_offset, int stage) {
	size_t count = 0;
	int to_square = add_offset(from_square, file_offset, rank_offset);

	if (to_square != NO_SQUARE) {
		int piece = pos->board[to_square];
		int iscapture = (piece != NO_PIECE && COLOR(piece) != pos->side_to_move);
	
		if ((stage == 0 && iscapture) || (stage == 1 && !iscapture)) {
			moves[count++] = make_move(from_square, to_square, NO_TYPE);
		}
	}
	return count;
...
static size_t generate_sliding_move(const struct position *pos, struct move *moves, int from_square, int file_offset, int rank_offset, int stage) {
	size_t count = 0;
	int to_square = add_offset(from_square, file_offset, rank_offset);
	...
	for (square = 0; square < 64; square++) 
	{
		...
		case PAWN:
			...
			if (stage == 0)
			{
				...
				count += generate_pawn_capture(pos, moves + count, square, 1, forward);
				/*Stage 0.2 - Promote*/
				if (up != NO_SQUARE && pos->board[up] == NO_PIECE && RANK(up) == RELATIVE(RANK_8, pos->side_to_move))
					count += generate_pawn_move(pos, moves + count, square, up);
			}
			else 
			{ /* Stage 1 - quiet pushes */
				if (up != NO_SQUARE && pos->board[up] == NO_PIECE && RANK(up) != RELATIVE(RANK_8, pos->side_to_move))
					count += generate_pawn_move(pos, moves + count, square, up_up);						
				if (RANK(square) == RELATIVE(RANK_2, COLOR(pos->board[square])) && up_up != NO_SQUARE && pos->board[up_up] == NO_PIECE) 
					count += generate_pawn_move(pos, moves + count, square, up_up);
			}		
			break;
		case KNIGHT:
			...
			if (stage == 0)
			{
				count += generate_simple_move(pos, moves + count, square, -1, -2, 0);
				count += generate_simple_move(pos, moves + count, square, 1, -2, 0);
				count += generate_simple_move(pos, moves + count, square, -2, -1, 0);
				count += generate_simple_move(pos, moves + count, square, 2, -1, 0);
				count += generate_simple_move(pos, moves + count, square, -2, 1, 0);
				count += generate_simple_move(pos, moves + count, square, 2, 1, 0);
				count += generate_simple_move(pos, moves + count, square, -1, 2, 0);
			...
			{
				count += generate_simple_move(pos, moves + count, square, -1, -2, 1);
				count += generate_simple_move(pos, moves + count, square, 1, -2, 1);
				count += generate_simple_move(pos, moves + count, square, -2, -1, 1);
				count += generate_simple_move(pos, moves + count, square, 2, -1, 1);
				count += generate_simple_move(pos, moves + count, square, -2, 1, 1);
				count += generate_simple_move(pos, moves + count, square, 2, 1, 1);
				count += generate_simple_move(pos, moves + count, square, -1, 2, 1);
		...
		if (stage == 0)
		{
			...
			if (TYPE(piece) != ROOK) {
				count += generate_sliding_move(pos, moves + count, square, -1, -1, 0);
				count += generate_sliding_move(pos, moves + count, square, 1, -1, 0);
				count += generate_sliding_move(pos, moves + count, square, -1, 1, 0);
				count += generate_sliding_move(pos, moves + count, square, 1, 1, 0);
			}
			...
			if (TYPE(piece) != BISHOP) {
				count += generate_sliding_move(pos, moves + count, square, 0, -1, 0);
				count += generate_sliding_move(pos, moves + count, square, -1, 0, 0);
				count += generate_sliding_move(pos, moves + count, square, 1, 0, 0);
				count += generate_sliding_move(pos, moves + count, square, 0, 1, 0);
			}
		}
		else
		{
			/* bishop and queen moves.                                       */
			if (TYPE(piece) != ROOK) {
				count += generate_sliding_move(pos, moves + count, square, -1, -1, 1);
				count += generate_sliding_move(pos, moves + count, square, 1, -1, 1);
				count += generate_sliding_move(pos, moves + count, square, -1, 1, 1);
				count += generate_sliding_move(pos, moves + count, square, 1, 1, 1);
			}
			...
			/* rook and queen moves.                                         */
			if (TYPE(piece) != BISHOP) {
				count += generate_sliding_move(pos, moves + count, square, 0, -1, 1);
				count += generate_sliding_move(pos, moves + count, square, -1, 0, 1);
				count += generate_sliding_move(pos, moves + count, square, 1, 0, 1);
				count += generate_sliding_move(pos, moves + count, square, 0, 1, 1);
			}
		}
		...
		if (stage == 0)
		{
			count += generate_simple_move(pos, moves + count, square, -1, -1, 0);
			count += generate_simple_move(pos, moves + count, square, 0, -1, 0);
			count += generate_simple_move(pos, moves + count, square, 1, -1, 0);
			count += generate_simple_move(pos, moves + count, square, -1, 0, 0);
			count += generate_simple_move(pos, moves + count, square, 1, 0, 0);
			count += generate_simple_move(pos, moves + count, square, -1, 1, 0);
			count += generate_simple_move(pos, moves + count, square, 0, 1, 0);
			count += generate_simple_move(pos, moves + count, square, 1, 1, 0);
		...
		{
			count += generate_simple_move(pos, moves + count, square, -1, -1, 1);
			count += generate_simple_move(pos, moves + count, square, 0, -1, 1);
			count += generate_simple_move(pos, moves + count, square, 1, -1, 1);
			count += generate_simple_move(pos, moves + count, square, -1, 0, 1);
			count += generate_simple_move(pos, moves + count, square, 1, 0, 1);
			count += generate_simple_move(pos, moves + count, square, -1, 1, 1);
			count += generate_simple_move(pos, moves + count, square, 0, 1, 1);
			count += generate_simple_move(pos, moves + count, square, 1, 1, 1);
...
size_t generate_legal_moves(const struct position *pos, struct move *moves, int stage) {
	size_t pseudo_legal_count = generate_pseudo_legal_moves(pos, moves, stage);
	size_t index;
	size_t count = 0;
...
void sort_moves(struct move *moves, size_t count, int stage)
{
	if (stage == 0)
		qsort(moves, count, sizeof(struct), )
}
